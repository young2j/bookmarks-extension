...
const Stepper = () => {
  return (
    <section className="w-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] sm:bg-[length:800px_400px] bg-[length:400px_200px] bg-no-repeat bg-top from-zinc-400/10 to-60% to-transparent animate-fade animate-delay-200">
      <div className="mb-16 animate-fade-up animate-delay-500 animate-duration-500">
        <div className="flex flex-col items-center gap-4 my-12">
          <h2 className="max-w-md text-4xl font-bold text-center text-transparent md:max-w-xl md:text-5xl from-zinc-400 via-zinc-200 to-zinc-400 bg-gradient-to-r bg-clip-text">
        ...
        <ul className="grid w-full grid-cols-1 gap-8 sm:grid-cols-3">
          <ListItem
            ...
            title="Add websites"
            description="Just paste or type the URL of the website you want to save, and your bookmark will be instantly stored."
          />
          <ListItem
            icon={<FilterList width={18} />}
            title="Tag bookmarks"
            description="Add custom tags to each bookmark, making it a breeze to find and access your most treasured pages."
          />
          <ListItem
            icon={<Search width={18} />}
            title="Filter and search"
            description="Easily filter your bookmarks by tags or experience lightning-fast searching with the command menu."
            withDivider={false}
          />
        </ul>
      </div>
...
const ListItem = ({
  ...
  return (
    <li className="relative flex sm:flex-col sm:items-center sm:text-center group">
      {withDivider && (
        <span
          className="absolute left-5 top-12 h-[calc(100%-24px)] w-px sm:h-px sm:top-5 sm:left-[calc(50%+30px)] sm:w-[calc(100%-30px)] bg-zinc-700"
